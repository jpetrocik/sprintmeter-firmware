...
public class BluetoothSerial {
	...
	/**
	 * Listens for discount message from bluetooth system and try reestablishing a connection
	 */
		...
		public void run() {
			...
			while (!isInterrupted()) {
				try {
					...
					/*
					 * check for some bytes, or bytes still left in
					 * buffer
					 */
					if (available() > 0){ 
...
//						Log.v(BMX_BLUETOOTH, "read " + newBytes);
					}
	...
	static public Dialog connectionDialog(Context context){
