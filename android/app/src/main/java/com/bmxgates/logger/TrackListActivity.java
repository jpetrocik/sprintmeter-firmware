 ...
 */
public class TrackListActivity extends FragmentActivity implements TrackListFragment.Callbacks {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.track_detail_container) != null) {
			...
			((TrackListFragment) getSupportFragmentManager().findFragmentById(R.id.track_list)).setActivateOnItemClick(true);
		}
	...
	public void onItemSelected(int trackId) {
		if (mTwoPane) {
			...
			arguments.putInt(TrackDetailFragment.ARG_TRACK, trackId);
			TrackDetailFragment fragment = new TrackDetailFragment();
			fragment.setArguments(arguments);
			getSupportFragmentManager().beginTransaction().replace(R.id.track_detail_container, fragment).commit();

		} else {
			// In single-pane mode, simply start the detail activity
			// for the selected item ID.
			Intent detailIntent = new Intent(this, TrackDetailActivity.class);
			detailIntent.putExtra(TrackDetailFragment.ARG_TRACK, trackId);
